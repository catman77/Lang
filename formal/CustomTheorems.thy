...
theorem reachability_transitive:
  shows "reachable R s t \<and> reachable R t u \<longrightarrow> reachable R s u"
  sorry  (* Induction proof required *)

theorem length_bounded:
  shows "length s \<le> max_len \<longrightarrow> length (rewrite s) \<le> max_len + k"
  sorry  (* Manual proof required *)

theorem deterministic_rewriting:
  shows "reachable R s a \<and> reachable R s b \<and> is_deterministic R \<longrightarrow> a = b"
